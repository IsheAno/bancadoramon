    ...
    private function _addDataToBlock($block)
    {
        ...
        if (array_key_exists('added_product', $actionData)) {
            if (isset($actionData['parent_product'])) {
                ...
                $block->setData('product_id', $actionData['parent_product']);
            } else {
            ...
            if (null === $promo && isset($actionData['parent_product'])) {
                $promo = Mage::helper('ajaxcartpro/promo')->validate(
                    $actionData['parent_product'], AW_Ajaxcartpro_Model_Source_Promo_Rule_Type::ADD_VALUE
                );
            }
            if (null !== $promo) {
