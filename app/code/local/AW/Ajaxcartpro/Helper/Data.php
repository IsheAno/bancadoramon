...
{
    protected $_previewSecurityKeyName = '_aw_acp_preview_security_key';
    ...
    public function addAjaxcartproVariablesToWysiwygConfig($config)
    {
        ...
        $newUrl = Mage::getUrl('adminhtml/awajaxcartpro_system_ajax/wysiwygPluginVariables');
        if (Mage::app()->getStore()->isCurrentlySecure()) {
            $newUrl = Mage::getUrl(
                'adminhtml/awajaxcartpro_system_ajax/wysiwygPluginVariables',
                array('_secure' => true)
            );
        }
        $options['url'] = $newUrl;
    ...
    public function getPreviewSecurityKey()
    {
        $session = Mage::getSingleton('core/session', array('name' => 'adminhtml'));
        $securityKey = $session->getData($this->_previewSecurityKeyName);
        if (!$securityKey) {
            $securityKey = Mage::helper('core')->getRandomString(16);
            $session->setData($this->_previewSecurityKeyName, $securityKey);
        }
        return $securityKey;
    }
}
