 ...
 */
class Wyomind_Massstockupdate_Block_Adminhtml_Import_Renderer_Status extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    ...
    {
        $dir = Mage::getBaseDir() . DS . 'var' . DS . 'flag' . DS;
        $file = $dir . $this->module . "_" . $row->getId() . ".flag";
        ...
        if ($flag->fileExists($file, false)) {
            ...
            if ($line[0] == $this::_SUCCEEDED) {
                $line[0] = $this->checkCronTasks($line[0], $row, $stats["mtime"]);
            }

            switch ($line[0]) {
                case $this::_SUCCEEDED:
                    $severity = 'notice';
                    $status = Mage::helper($this->module)->__($line[0]);
                    break;
                case $this::_PENDING:
                    $severity = 'minor';
                    $status = Mage::helper($this->module)->__($line[0]);
                ...
                case $this::_COLLECTING:
                    $percent = $line[2];
                    $severity = 'minor';
                    $status = Mage::helper($this->module)->__($line[0]) . " [" . $percent . "%]";
                    break;
                ...
                default :
                    $severity = 'critical';
                    $status = Mage::helper($this->module)->__("ERROR");
                    break;
            }
        } else {
            $severity = 'minor';
            $line[1] = "no message";
            $status = Mage::helper($this->module)->__($this::_PENDING);
        }
        $script = "<script language='javascript' type='text/javascript'>var updater_url='"
                . $this->getUrl('*/' . $this->module . '/updater') . "'</script>";

        return $script . "<span title='" . $line[1] . "' class='grid-severity-$severity updater' cron='" . $row->getCronSetting()
    ...
    protected function checkCronTasks($status, Varien_Object $row, $mtime)
    {
        ...
        $cronExpr = json_decode($row->getCronSetting());
        $i = 0;
